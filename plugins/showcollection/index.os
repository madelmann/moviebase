...
public object RenderPlugin implements IRenderPlugin {
	...
	private int getNumPages() throws {
		string query = "SELECT count(*) AS num_items FROM collection_items WHERE collection_id = " + mCollectionID;
		...
		if ( mysql_fetch_row( result ) ) {
			int numItems = int mysql_get_field_value( result, "num_items" );
	...
	private void ShowVideos( int page ) throws {
		...
		string query = "SELECT @rownum := (@rownum + 1) AS num, i.actors, i.filename, i.id, i.md5sum, i.rating_count, i.rating_value, i.title, i.views, ci.collection_id, ci.item_id "
		...
		while ( mysql_fetch_row( result ) ) {
			...
			string collectionItemID = cast<int>( mysql_get_field_value( result, "num" ) ) - 1;
			string id = mysql_get_field_value( result, "id" );
			string md5sum = mysql_get_field_value( result, "md5sum" );
			int num = cast<int>( mysql_get_field_value( result, "num" ) );
